...
$hasta=$h[2]."-".$h[1]."-".$h[0];
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE maquina='Fresadora' AND m.n_b=n.id_nb AND estatus='1' AND modificado BETWEEN  '".$desde."' AND '".$hasta."'");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1'   AND maquina='Fresadora' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1'  AND maquina='Fresadora' AND estado='No Operativo'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <tr>
                <td>
                    Nº de Fresadoras Encontradas: <?php echo $columnas; ?>
 
                </td>
            </tr>
             <tr>
                <td>
                    Nº Total de Fresadoras <?php echo $fresadora; ?>
...
$html2pdf -> Output('reporte_fresadora_rango_'.$d.'-'.$m.'-'.$y.'.pdf','I');
?>