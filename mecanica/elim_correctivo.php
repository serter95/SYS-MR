<?php
 ...
 #   echo $id;
    ...
    pg_query("UPDATE mant_correctivo SET estatus='0' WHERE id_correctivo='".$id."'");
    //pg_query("DELETE FROM maquinas WHERE id_maquina='".$id."'");
#   Actualizamos los rerigistros y los obtenemos

    $sql="SELECT * FROM mant_correctivo WHERE estatus='1' ";
    $query=pg_query($sql);
    ...
    pg_query("INSERT INTO auditoria (fecha,accion_realizada,modulo,nom_usuario,taller)
        VALUES('$hoy','Eliminación','Mantenemiento Correctivo','".$_SESSION['nom_usuario']."',1)");

    header("Location:mant_correctivo.php?elim_maq=si");
