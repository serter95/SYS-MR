...
/*$sql="SELECT * FROM maquinas AND estatus=1";
$query=pg_query($sql);  
$array=pg_fetch_assoc($query);
*/
//if($array['codigo']!=$codigo)
//{
/*if (!eregi("^[A-Za-z0-9-]{4,8}$",$codigo)){
?>
	<script language="javascript">
		alert("Se coloco una expresion diferente a un numero en el campo de codigo");
...
else if (!eregi("^[A-Za-záéíóúñÁÉÍÓÚÑ0-9.-]{3,10}$",$marca)){
?>
	<script language="javascript">
...
else if (!eregi("^[A-Za-záéíóúñÁÉÍÓÚÑ0-9.-]{4,10}$",$modelo)){
?>
	<script language="javascript">
...
//-----------------------DESDE Aqui insert individual---------------------------
if ($maquina == "Fresadora"){
				
				$sqf = "SELECT * FROM maquinas WHERE codigo LIKE 'Fr-%'  ORDER BY id_maquina ASC";
				$resul = pg_query($sqf);
...
elseif ($maquina == "Torno"){
				
				$sqf = "SELECT * FROM maquinas WHERE codigo LIKE 'To-%' ORDER BY id_maquina ASC";
				$resul = pg_query($sqf);
				while ($row = pg_fetch_assoc($resul)) {
					$m = $row['codigo'];
				}
...
elseif ($maquina == "Limadora"){
				
				$sqf = "SELECT * FROM maquinas WHERE codigo LIKE 'Li-%' ORDER BY id_maquina ASC";
				$resul = pg_query($sqf);
				while ($row = pg_fetch_assoc($resul)) {
					$m = $row['codigo'];
				}
...
elseif ($maquina == "Esmeril"){
				
				$sqf = "SELECT * FROM maquinas WHERE codigo LIKE 'Es-%' ORDER BY id_maquina ASC";
				$resul = pg_query($sqf);
				while ($row = pg_fetch_assoc($resul)) {
					$m = $row['codigo'];
				}
		...
		elseif ($maquina == "Taladro"){
				
				$sqf = "SELECT * FROM maquinas WHERE codigo LIKE 'Ta-%'  ORDER BY id_maquina ASC";
				$resul = pg_query($sqf);
				while ($row = pg_fetch_assoc($resul)) {
					$m = $row['codigo'];
				}
...
//--------------Pequeña validacion existe nb php-----------
		$buscanb="SELECT * FROM numero_bien WHERE nb='".$nb."'";
