<?php
	...
	if ($existe=='si')
	{
		header('Location:tecnologico.php?error=si');
	}
	else
	{
		if (empty($nac2) || empty($ced2) || empty($nomEst2) || empty($apeEst2) || empty($cod2) || empty($tlf2) || empty($correoEst2) || empty($especialidad2))
		{
				pg_query("INSERT INTO proyectos (codigo,regimen,estado,municipio,parroquia,sector,pnf,trayecto,seccion
					...
					,'$periodo','$titulo','$descripcion','$aportes','$integracion','$planNacional','tecnologico','En proceso','2')");
			    ...
			    VALUES('$hoy','Registro','Tecnológico','".$_SESSION['nom_usuario']."')");

				header("Location:tecnologico.php?registro=exito");
		}
