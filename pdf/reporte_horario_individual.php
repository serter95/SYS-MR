...
$datos=pg_query("SELECT * FROM horario_clase hc, horas h, materia m, personal p WHERE hc.id_horas=h.id_horas AND hc.id_materia=m.id_materia AND hc.id_personal=p.id AND hc.id_personal='$id' AND hc.estatus=1 AND hc.id_periodo='$periodo' AND h.estatus=1 AND m.estatus=1 ORDER BY h.numero_bloque ASC");
...
?>
    ...
    $query2=pg_query("SELECT * FROM horas WHERE estatus=1 AND taller='".$_REQUEST['pagina']."' ORDER BY numero_bloque ASC");
