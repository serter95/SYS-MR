...
class pgBackupRestore
{
   ...
   function Backup($uiFilename = NULL)
   {
      ...
      //esto convierte la toda la pagina o archivo recibido y lo convierte en un archivo a descarga
...
readfile($this->Filename);
      ...
      //despues de creado el fichero lo borra con unlink
  ...
  // copy($this->Database.".sql");
      ...
      if (!is_readable($this->Filename)){
        $this->Error("Can't find {$this->Filename} for opening", true);// header("Location:../BD.php?error=si");
               echo "<script language='javascript'>window.location='../BD.php?error=si'</script>";
      ...
      while ( $_readSQL = fgets($_fpSQL) )
      {
         ...
         if ($this->GotSQLerror)
            $this->Error("SQL syntax error on line ${_CurrentLine} (". $this->LastSQLerror .")", true); //header("Location:../BD.php?error=nocompatible");
      }
       echo "<script language='javascript'>window.location='../BD.php?realizado=si'</script>";
   }
