<?php
 ...
 #   echo $id;
    ...
    pg_query("UPDATE mant_preventivo SET estatus='0' WHERE id_preventivo='".$id."'");
    //pg_query("DELETE FROM maquinas WHERE id_maquina='".$id."'");
#   Actualizamos los rerigistros y los obtenemos

    $sql="SELECT * FROM mant_preventivo WHERE estatus='1' ";
    $query=pg_query($sql);
    ...
    pg_query("INSERT INTO auditoria (fecha,accion_realizada,modulo,nom_usuario,taller)
        VALUES('$hoy','Eliminación','Mantenemiento Preventivo','".$_SESSION['nom_usuario']."',1)");

    header("Location:mant_preventivo.php?elim_maq=si");
