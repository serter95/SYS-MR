...
//consultax=consulta.charAt(0).toUpperCase()+consulta.slice(1);
   ...
   $.ajax({
      ...
      if (valor2==1)
    {
       ...
       mostrarPrompt("Ya en uso", "SerialPrompt", "red");
   ...
   if (valor2==0)
    {
      ...
      if(!code.match(/^[0-9]{4,6}$/)){
        mostrarPrompt("Invalido", "SerialPrompt", "red");
 ...
 var id= document.getElementById("idmod").value;
   ...
   $.ajax({
      ...
      if (valor2==0)
    {
       ...
       mostrarPrompt("Ya en uso", "SerialmodPrompt", "red");
   ...
   if (valor2==1)
    {
      ...
      if(!code.match(/^[0-9]{4,6}$/)){
        mostrarPrompt("Invalido", "SerialmodPrompt", "red");
...
function validarmodform(){
   if( !validarM_N_B() || !validarM_Nombre() || !existeCodigo2() || !validarM_Ubicacion() || !existeSerial2() || !validarM_Marca()){
...
function validarForm()
{
  ...
  if( !existeCodigo() || !validarNombre() || !validarN_B() || !existeSerial() || !validarMarca() || !validarUbicacion()){
...
{
  //validaCodigo dunciona para todos WTF  aqui iba !validarCodigo() ||
  if(!validarCI() || !validarFecha() || !validarFecha2() || !validarResponsable() ){
     /*   $('#error_incompleto').modal({
                    show:true,
                    backdrop:'static'
                }).show(200);*/
...
{
  //validaCodigo dunciona para todos WTF  aqui iba !validarCodigo() ||
  if( !existeNB() || !validarCI() || !validarFecha() || !validarFecha2() || !validarResponsable() ){
        $('#error_incompleto').modal({
                    show:true,
                    backdrop:'static'
                }).show(200);
