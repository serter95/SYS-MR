...
$maq=$_REQUEST['maq'];
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE m.n_b=n.id_nb AND m.maquina='$maq' AND m.estatus='1'  AND modificado BETWEEN  '".$desde."' AND '".$hasta."' ORDER BY m.codigo ASC");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1'   AND maquina='$maq' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1'  AND maquina='$maq' AND estado='No Operativo'");

$inoperativos=pg_num_rows($total_nope);


$total_esmeril=pg_query("SELECT maquina FROM maquinas WHERE estatus='1' AND maquina='$maq'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <tr>
                <td>
                    Nº de <?php echo $maq; ?> Encontrados: <?php echo $columnas; ?>
            ...
            <tr>
                <td>
                    Nº de <?php echo $maq; ?> Operativas: <?php echo $operativos; ?> 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de <?php echo $maq; ?> Inoperativas: <?php echo $inoperativos; ?>
                </td>
            </tr>
        </table>
    </fieldset>
...
$html2pdf -> Output('reporte_esmeril_rango_'.$d.'-'.$m.'-'.$y.'.pdf','I');
?>