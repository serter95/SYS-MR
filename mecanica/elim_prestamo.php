<?php
    ...
    $id2=$_REQUEST['id2']; //herramienta
 #   echo $id;

    # Eliminamos logicamente el usuario

    pg_query("UPDATE prestamo SET estatus='0' WHERE id_prestamo='".$id."'");
     pg_query("UPDATE herramientas SET estado= 'Operativo' WHERE id_herramienta='".$id2."'");
    //pg_query("DELETE FROM maquinas WHERE id_maquina='".$id."'");
#   Actualizamos los rerigistros y los obtenemos

    $sql="SELECT * FROM prestamo WHERE estatus='1'";
    $query=pg_query($sql);
    ...
    date_default_timezone_set('America/Caracas');
          ...
          pg_query("INSERT INTO auditoria (fecha,accion_realizada,modulo,nom_usuario,taller)
              VALUES('$hoy','Eliminación','Prestamo','".$_SESSION['nom_usuario']."',1)");

    header("Location:prestamos.php?elim_maq=si");
