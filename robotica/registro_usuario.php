<?php
	...
	$sql="SELECT * FROM personal WHERE ci='$ci' AND estatus=1 AND area='Robotica'";
	$query=pg_query($sql);
	$dato=pg_fetch_assoc($query);
	$id_per=$dato['id'];
	
	$sql_2="SELECT * FROM usuario WHERE taller=2 AND estatus=1";
	...
	if ($iguales!='si')
	{
		...
		$sql="SELECT * FROM usuario WHERE nom_usuario='$nom_usuario' AND taller=2 AND estatus=1 LIMIT 1";
		$query=pg_query($sql);
		...
		if ($array['nom_usuario']!=$nom_usuario)
		{
			...
			$inst="SELECT * FROM tipo_usuario WHERE tipo='$tipo' AND area In ('Robotica' , 'Doble')";
			$con=pg_query($inst);
			$dato=pg_fetch_assoc($con);
			$id_tipo_usuario=$dato['id_tipo'];
				
			$sql="INSERT INTO usuario (nom_usuario, contrasena, estatus, id_personal, id_tipo_usuario, pregunta, respuesta,taller)
				VALUES ('$nom_usuario', '$contrasena', 1, '$id_per', '$id_tipo_usuario', '$pregunta', '$respuesta','2')";
			    ...
			    VALUES('$hoy','Registro','Usuarios','".$_SESSION['nom_usuario']."')");
