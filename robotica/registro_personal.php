<?php
	...
	$sql="SELECT * FROM personal WHERE estatus=1 AND area='Robotica'";
	$query=pg_query($sql);
	...
	if ($com_ci=='no')
	{
		if ($com_correo=='no')
		{
			...
			$sql="INSERT INTO personal(ci,nombres,apellidos,sexo,grado_instruccion,especialidad,area,cargo,correo,numero_contacto,fecha_nacimiento,estatus,dedicacion)
				VALUES('$nac_ci','$nombres','$apellidos','$sexo','$grado_inst','$especialidad','Robotica','$cargo','$correo','$num_cont','$fecha_nac',1,'$dedicacion')";
			    ...
			    VALUES('$hoy','Registro','Personal','".$_SESSION['nom_usuario']."')");
