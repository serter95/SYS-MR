<?php
    ...
    for ($i=0; $i < $num ; $i++)
    {
        ...
        AND h.dia='".$dia."' AND pe.area='Mecanica' AND h.taller=1 AND h.estatus=1 
        ...
        AND h.id_personal2=pe.id AND p.tipo='$periodo' AND h.id_personal2='$id_personal[$i]' AND h.dia='".$dia."'
        ...
        AND h.id_personal3=pe.id AND p.tipo='$periodo' AND h.id_personal3='$id_personal[$i]' AND h.dia='".$dia."'
        AND pe.area='Mecanica' AND h.taller=1 AND h.estatus=1 AND pe.estatus=1 ORDER BY pe.ci ASC");
        ...
        {
            $q=pg_query("SELECT * FROM personal WHERE id='$id_personal[$i]'");
            $a=pg_fetch_assoc($q);
