...
function validarmodform(){
   if(!existeCodigo2() || !validarM_Nombre() || !validarM_N_B() || !opciontipomedidamod() || !validarMedidamod() || !validarPrecio2() ||!validarExistencia2() || !validarBuenas2() || !calculo_danada2()  || !min_stockmod() || !max_stockmod() || !validarUbicacion2()){
...
function opciontipomedidamod(){
  ...
  if (tipo=='No aplica')
  {
    ...
    document.getElementById('medida_pul_mod').disabled=true;
    document.getElementById('medida2_pul_mod').disabled=true;
  ...
  if(tipo=='Pulgadas'){
    ...
    document.getElementById('medidamod').disabled=true;
    document.getElementById('medida_pul_mod').disabled=false;
    document.getElementById('medida2_pul_mod').disabled=false;

    var medida1=$('#medida_pul_mod').val();
    var medida2=$('#medida2_pul_mod').val();

    $('#div_medidaM1').hide();
    $('#div_medidaM2').show();
    
    if (medida1=="" || medida2==""){
      mostrarPrompt("Campo Requerido", "medida2Prompt_mod", "red");
      return false;
    }
    if ((!medida1.match(/^[1-9]{1}([0-9]{1})?$/)) || (!medida2.match(/^[1-9]{1}([0-9]{1})?$/)))
    {
      mostrarPrompt("Invalido", "medida2Prompt_mod", "red");
      return false;
    }
    else
    {
      mostrarPrompt("Valido", "medida2Prompt_mod", "green");
      return true;
    }
  }
  if(tipo=='Milimetros'){
...
function validarForm()
{
  ...
  if(!validarUbicacion() || !existeCodigo() || !validarNombre() || !validarN_B()|| !validarMedida() || !validarPrecio() || !validarExistencia() || !validarBuenas() || !calculo_danada() || !min_stock() || !max_stock() ){
...
{
  //validaCodigo dunciona para todos WTF  aqui iba !validarCodigo() ||
  if( !existeCodigoE() || !validarExistenciaEntrada() || !validarBuenasEntrada() || !validarEntradaDanadas()){
        /*$('#error_incompleto').modal({
                    show:true,
                    backdrop:'static'
                }).show(200);
...
{
  //validaCodigo dunciona para todos WTF  aqui iba !validarCodigo() ||
  if( !existeCodigoS() ||  !validarExistenciaSalida() ||!validarBuenasSalida() || !validarSalidaDanadas() ){
        /*$('#error_incompleto').modal({
                    show:true,
                    backdrop:'static'
                }).show(200);
