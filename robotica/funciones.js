...
//  FIN CONFIGURACION
    ...
    function ValidarBloqueProfe (parametro) {
        ...
        if (prof!="")
        {
                ...
                if (ced!="" && aula!="" && materia!="" && seccion!="")
                {
                    ...
                    $.ajax({
                        ...
                        success: function(valores){
                            ...
                            {
                                jsMostrar("SeleccionBloque");
                                mostrarPrompt("El Profesor ya da otra materia en esta aula", "SeleccionBloque", "green");
            ...
            else
            {
                ...
                mostrarPrompt("Esta Persona No es Profesor", "SeleccionBloque", "red");
    ...
    function ValidarBloqueProfeM (parametro) {
        ...
        if (prof!="")
        {
                ...
                if (ced!="" && aula!="" && materia!="" && seccion!="")
                {
                    ...
                    $.ajax({
                        ...
                        success: function(valores){
                            ...
                            //$('#valorAjaxProfM').val(valores);

                            //var valor=document.getElementById('valorAjaxProfM').value;

                            //alert(valores);

                            if (valores==3)
                            {
                                //alert("El Profesor ya esta dando otra materia en esta aula");
                                document.getElementById('submitM').disabled=false;
                                jsMostrar("SeleccionBloqueM");
                                mostrarPrompt("El Profesor ya da otra materia en esta sección", "SeleccionBloqueM", "green");
                            }
            ...
            else
            {
                ...
                mostrarPrompt("Esta Persona No es Profesor", "SeleccionBloqueM", "red");
    ...
    function marcarBloque()
    {
        ...
        if (val[0]==0)
        {
            ...
            var aula1=aula.split(' ');
            var aula=aula1.join('-');

            $('#'+div).val(1+" "+materia+" "+aula+" "+ced);
        }
        else
        { 
            //alert("esta verde y debe cambiar a color azul");
        ...
        $.ajax({
            ...
            success: function(valores){
                ...
                for (var i=1; i<=datos[0]; i++)
                {
                    ...
                    var aula=datos[4][j].split(' ');
                    datos[4][j]=aula.join('-');
                
                    $('#'+datos[3][j]+'_M').val(1+" "+datos[6][j]+" "+datos[4][j]+" "+nom_ci[2]+" "+datos[7][j]);
                }
        ...
        $.ajax({
            ...
            success: function(valores){
                ...
                for (var i=1; i<=datos[0]; i++)
                {
                    ...
                    $('#'+datos[3][j]+'_D').val(1+" "+datos[6][j]+" "+datos[4][j]+" "+nom_ci[2]+" "+datos[7][j]);
        ...
        $.ajax({
            ...
            success: function(valores){
                ...
                if (datos[4]=='Pulgadas')
                {
                    ...
                    var medida=datos[5].split("/");

                    $('#medida_pul_mod').val(medida[0]);
                    $('#medida2_pul_mod').val(medida[1]);
                }
                if (datos[4]=='No aplica')
                {
                    $("#div_medidaM1").hide();    
                    $("#div_medidaM2").hide();
                }
