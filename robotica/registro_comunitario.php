<?php
	...
	$query=pg_query("SELECT * FROM proyectos WHERE estado_proyecto In ('Concluido','En proceso') AND taller=2 AND estatus=1");
	...
	else
	{
		if (empty($nac2) || empty($ced2) || empty($nomEst2) || empty($apeEst2) || empty($cod2) || empty($tlf2) || empty($correoEst2) || empty($especialidad2))
		{
				pg_query("INSERT INTO proyectos (codigo,regimen,estado,municipio,parroquia,sector,pnf,trayecto,seccion
					,periodo,titulo,descripcion,aportes,integracion,plan_nacional,ambito,estado_proyecto,taller) VALUES
					('$codigo','$regimen','$estado','$municipio','$parroquia','$sector','$pnf','$trayecto','$seccion'
					,'$periodo','$titulo','$descripcion','$aportes','$integracion','$planNacional','comunitario','En proceso','2')");

				$query=pg_query("SELECT * FROM proyectos WHERE taller=2 AND estatus=1 ORDER BY id_proyecto DESC");
			    ...
			    VALUES('$hoy','Registro','Comunitario','".$_SESSION['nom_usuario']."')");
		...
		{
					pg_query("INSERT INTO proyectos (codigo,regimen,estado,municipio,parroquia,sector,pnf,trayecto,seccion
						,periodo,titulo,descripcion,aportes,integracion,plan_nacional,ambito,estado_proyecto,taller)
		...
		if (empty(!$nac2) && empty(!$ced2) && empty(!$nomEst2) && empty(!$apeEst2) && empty(!$cod2) && empty(!$tlf2) && empty(!$correoEst2) && empty(!$especialidad2) && empty(!$nac3) && empty(!$ced3) && empty(!$nomEst3) && empty(!$apeEst3) && empty(!$cod3) && empty(!$tlf3) && empty(!$correoEst3) && empty(!$especialidad3))
		{
						...
						$query="SELECT * FROM proyectos WHERE taller=2 AND estatus=1 ORDER BY id_proyecto DESC";
