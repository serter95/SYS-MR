...
$hasta=$h[2]."-".$h[1]."-".$h[0];
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE maquina='Esmeril' AND m.n_b=n.id_nb AND estatus='1' AND modificado BETWEEN  '".$desde."' AND '".$hasta."'");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1'   AND maquina='Esmeril' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1'  AND maquina='Esmeril' AND estado='No Operativo'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <tr>
                <td>
                    Nº de Esmeriles Encontrados: <?php echo $columnas; ?>
 
                </td>
            </tr>
             <tr>
                <td>
                    Nº Total de Esmeriles: <?php echo $esmeril; ?>
 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Esmeriles Operativas: <?php echo $operativos; ?> 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Esmeriles Inoperativas: <?php echo $inoperativos; ?>
                </td>
            </tr>
        </table>
    </fieldset>
