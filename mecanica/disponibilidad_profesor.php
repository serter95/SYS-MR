<?php
	...
	$queryx=pg_query("SELECT id_materia FROM materia WHERE codigo='$materia' AND taller=1 AND estatus=1");
	...
	if ($diaSelecciondo=="disponible")
	{
		$con2=pg_query("SELECT * FROM horario_clase WHERE dia='".$_POST['div']."' AND id_periodo='$id_periodo'
		 AND id_personal='$id' AND taller=1 AND estatus=1");
		$num=pg_num_rows($con2);

		if ($num==0)
		{
			$con3=pg_query("SELECT * FROM horario_clase WHERE dia='".$_POST['div']."' 
			AND id_periodo='$id_periodo' AND aula='$aula' AND taller=1 AND estatus=1");
			$num2=pg_num_rows($con3);

			if ($num2==0)
			{
				$con4="SELECT * FROM horario_clase WHERE id_periodo='$id_periodo' AND id_seccion='$id_seccion' AND id_personal='$id' AND taller=1 AND estatus=1";
				$q4=pg_query($con4);
				$num3=pg_num_rows($q4);
