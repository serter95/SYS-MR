...
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE maquina='Limadora' AND m.n_b=n.id_nb AND estatus='1' AND modificado BETWEEN  '".$desde."' AND '".$hasta."'");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1'   AND maquina='Limadora' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1'  AND maquina='Limadora' AND estado='No Operativo'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <tr>
                <td>
                    Nº de Limadoras Encontradas: <?php echo $columnas; ?>
 
                </td>
            </tr>
             <tr>
                <td>
                    Nº Total de Limadoras <?php echo $limadora; ?>
 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Limadoras Operativas: <?php echo $operativos; ?> 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Limadoras Inoperativas: <?php echo $inoperativos; ?>
                </td>
            </tr>
        </table>
    </fieldset>
...
$html2pdf -> Output('reporte_limadora_rango_'.$d.'-'.$m.'-'.$y.'.pdf','I');
?>