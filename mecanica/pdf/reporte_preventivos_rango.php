...
//$datos = pg_query ($conexion, "SELECT * FROM usuario");
$datos=pg_query("SELECT * FROM maquinas m, mant_preventivo p, personal s, numero_bien n WHERE m.id_maquina=p.maquina_id AND m.n_b=n.id_nb AND p.id_personal=s.id  AND m.estatus='1' AND p.estatus='1' AND p.estado<>'en proceso'  AND fecha BETWEEN  '".$desde."' AND '".$hasta."' ORDER BY fecha DESC");
$datos2=pg_query("SELECT costo, fecha FROM mant_preventivo WHERE estatus='1' AND estado<>'en proceso'  AND fecha BETWEEN  '".$desde."' AND '".$hasta."' ORDER BY fecha DESC");
...
?>
    ...
    <br>
      <div class="center">
        <h3 >Reporte de Mantenimiento Preventivo</h3>
    </div>
    ...
    <table class="tabledate">
         <tr>
            ...
            <td border="1" class="td_title" width="100">Número del Bien </td>
             <td border="1" class="td_title" width="100">Encargado</td>
              <td border="1" class="td_title" width="100">Responsable</td>
            <td border="1" class="td_title" width="50">Nivel de Aceite</td>
            <td border="1" class="td_title" width="60">Luces T.</td>
             <td border="1" class="td_title" width="70">P. Emergencia</td>
              <td border="1" class="td_title" width="70">Pulsadores</td>
...
<?php
        ...
        while($fila=pg_fetch_assoc($datos)){
                  $fecha_a=explode('-', $fila['fecha_ejecucion']);
                            $ano=$fecha_a[0];
                            $mes=$fecha_a[1].'/';
                            $dia=$fecha_a[2].'/';
                            $fecha=$dia.$mes.$ano;
            ...
            <tr>
                ...
                <td border="1" style="text-align: center;" width="50"><?php echo $fila['nivel_aceite'];?></td>
                <td border="1" style="text-align: center;" width="60"><?php echo $fila['luces_tablero'];?></td>
                <td border="1" style="text-align: center;" width="70"><?php echo $fila['parada_emergencia'];?></td>
                <td border="1" style="text-align: center;" width="70"><?php echo $fila['pulsadores'];?></td>
...
$html2pdf -> Output('reporte_mantenimiento_preventivo_rango_'.$d.'-'.$m.'-'.$y.'.pdf','I');
?>