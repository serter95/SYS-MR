...
//$datos = pg_query ($conexion, "SELECT * FROM usuario");
$datos=pg_query("SELECT * FROM maquinas m, mant_preventivo p, personal s, numero_bien n WHERE m.id_maquina=p.maquina_id AND m.n_b=n.id_nb AND p.id_personal=s.id  AND m.estatus='1' AND p.estatus='1' AND p.maquina_id='$id' AND p.estado<>'en proceso'  AND fecha_ejecucion BETWEEN  '".$desde."' AND '".$hasta."' ORDER BY fecha_ejecucion DESC");
$datos2=pg_query("SELECT costo, fecha_ejecucion FROM mant_preventivo WHERE estatus='1' AND maquina_id='$id' AND estado<>'en proceso'  AND fecha_ejecucion BETWEEN  '".$desde."' AND '".$hasta."' ORDER BY fecha_ejecucion DESC");
