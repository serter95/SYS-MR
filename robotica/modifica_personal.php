<?php
	...
	$sql="SELECT * FROM personal WHERE ci='$nac_ci' AND estatus=1 AND area='Robotica'";
	$query=pg_query($sql);
	$num=pg_num_rows($query);
	$array=pg_fetch_assoc($query);

	$query2=pg_query("SELECT * FROM personal WHERE correo='$correo' AND estatus=1 AND area='Robotica'");
	...
	if ($num>0)
	{
		if ($id==$array['id'])
		{
			...
			if ($num2>0)
			{
				...
				{
					# UPDATE
					$sql="UPDATE personal SET ci='".$nac_ci."',nombres='".$nombres."',apellidos='".$apellidos."',sexo='".$sexo."',grado_instruccion='".$grado_inst."',especialidad='".$especialidad."',area='Robotica',cargo='".$cargo."',correo='".$correo."',numero_contacto='".$num_cont."',fecha_nacimiento='".$fecha_nac."',estatus=1,dedicacion='$dedicacion' WHERE id='".$id."'";
				    ...
				    VALUES('$hoy','Modificación','Personal','".$_SESSION['nom_usuario']."')");
