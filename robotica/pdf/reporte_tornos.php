...
$hasta=$h[2]."-".$h[1]."-".$h[0];
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE maquina='Torno' AND m.n_b=n.id_nb AND estatus='1' AND modificado BETWEEN  '".$desde."' AND '".$hasta."'");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1'   AND maquina='Torno' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1'  AND maquina='Torno' AND estado='No Operativo'");
...
$total_esmeril=pg_query($conexion, "SELECT maquina FROM maquinas WHERE estatus='1' AND maquina='Esmeril'");

$esmeril=pg_num_rows($total_esmeril);

$total_limadora=pg_query($conexion, "SELECT maquina FROM maquinas WHERE estatus='1' AND maquina='Limadora'");

$limadora=pg_num_rows($total_limadora);

$total_fresadora=pg_query($conexion, "SELECT maquina FROM maquinas WHERE estatus='1' AND maquina='Fresadora'");

$fresadora=pg_num_rows($total_fresadora);

$total_taladro=pg_query($conexion, "SELECT maquina FROM maquinas WHERE estatus='1' AND maquina='Taladro'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <tr>
                <td>
                    Nº de Tornos Encontrados: <?php echo $columnas; ?>
 
                </td>
            </tr>
             <tr>
                <td>
                    Nº Total de Tornos: <?php echo $tornos; ?>
 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Tornos Operativas: <?php echo $operativos; ?> 
                </td>
            </tr>
            <tr>
                <td>
                    Nº de Tornos Inoperativas: <?php echo $inoperativos; ?>
                </td>
            </tr>
        </table>
    </fieldset>
...
$html2pdf -> Output('reporte_tornos_rango_'.$d.'-'.$m.'-'.$y.'.pdf','I');
?>