...
$hasta=$h[2]."-".$h[1]."-".$h[0];
$datos=pg_query("SELECT * FROM maquinas m, numero_bien n WHERE m.n_b=n.id_nb AND m.estatus='1' AND m.modificado BETWEEN  '".$desde."' AND '".$hasta."'");
...
$total_ope=pg_query("SELECT * FROM maquinas WHERE estatus='1' AND estado='Operativo'");
//$totales = pg_num_rows($datos);
$operativos=pg_num_rows($total_ope);

$total_nope=pg_query("SELECT * FROM maquinas WHERE estatus='1' AND estado='No Operativo'");
...
<form>
    <fieldset id="fieldset_izquierdo">
        ...
        <table>
            <tr>
                <td>
                    Nº de Máquinas Encontradas: <?php echo $columnas; ?>
